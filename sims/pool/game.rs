//! The pool, live: rasterized on the GPU through egui's wgpu backend.
//!
...
use kosm::fluid::{box_half, Caustic, PoolGeometry, Surface, STAND_RISE, STAND_ROWS, STAND_TREAD};
use crate::pool;
use kosm::fluid::Droplet;
use kosm_view::{Camera, read_back};
use tang::Vec3 as V;
...
/// The reference tracer's view for a camera. The camera itself is generic and
/// lives in `kosm-view`; what it means to the pool is here.
pub fn view(camera: &Camera, width: u32, height: u32) -> pool::View {
    pool::View { eye: camera.eye, target: camera.target, width, height, vfov: camera.vfov }
}
