...
|---|---|---|
| coarse state is the world | spectral far field in (η, ψ) with H conserved to 1e-13, 2nd-order Zakharov terms conserving H₂+H₃ to 8e-5; rigid melon; height-field surface; block-sparse MPM grid (dense block table, indirect dispatch; 11% tax at a full box) | fine region is a box, not a residual — the sparse grid is the storage for the residual, not yet used as one |
