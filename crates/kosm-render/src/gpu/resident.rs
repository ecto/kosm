...
impl EnvTextures {
    fn new(ctx: &GpuContext, scene: SceneRef<'_>) -> Self {
    ...
    fn signature_of(scene: SceneRef<'_>) -> Option<(u32, u32)> {
        match scene.environment {
...
pub struct ResidentScene {
    /// The client's geometry slabs, bindings 1..=n in declaration order.
    geometry: Vec<Slab>,
    materials: Slab,
    lights: Slab,
    env: EnvTextures,
...
impl ResidentScene {
    fn build(ctx: &GpuContext, scene: SceneRef<'_>, width: u32, height: u32) -> Self {
        ...
        let mut me = Self {
            geometry: scene
                ...
                .slabs()
                .iter()
                .map(|slab| Slab::new(ctx, slab.label, slab.bytes))
                .collect(),
            materials: Slab::new(
                ctx,
                "Resident Materials",
    ...
    fn refresh_scene_state(&mut self, scene: SceneRef<'_>) {
    ...
    pub fn update_scene<'s>(&mut self, ctx: &GpuContext, scene: impl Into<SceneRef<'s>>) {
        let scene: SceneRef<'s> = scene.into();
        let mut stale = false;
        let slabs = scene.geometry.slabs();
        // A module's binding count is fixed by its WGSL, so this only ever
        // rewrites in place or grows a slab; it never adds or drops one.
        debug_assert_eq!(slabs.len(), self.geometry.len());
        for (dst, src) in self.geometry.iter_mut().zip(slabs.iter()) {
            stale |= dst.write(ctx, src.label, src.bytes);
        }
    ...
    fn bind_entries<'a>(&'a self, output: &'a wgpu::TextureView) -> Vec<wgpu::BindGroupEntry<'a>> {
        [
            wgpu::BindGroupEntry {
                binding: 0,
        ...
        .chain(
            self.geometry
                .iter()
                .enumerate()
                .map(|(i, slab)| wgpu::BindGroupEntry {
                    binding: 1 + i as u32,
...
impl RayTracePipeline {
    ...
    pub fn resident_scene<'s>(
        ...
        let _ = self;
        ResidentScene::build(ctx, scene.into(), width, height)
    }
