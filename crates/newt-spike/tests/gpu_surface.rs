...
use newt_spike::pool::{self, Surface};
use newt_spike::splash::{HeightGrid, Water};
...
fn caustic_matches_the_cpu() {
    ...
    let mut gpu_dev = newt_mpm::GpuCaustic::new().expect("gpu");
...
fn caustic_flux_probe() {
    ...
    let cpu = pool::caustic(&surface, cell);
    let mut dev = newt_mpm::GpuCaustic::new().expect("gpu");
