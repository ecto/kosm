...
pub fn cand_cap() -> u32 {
    std::env::var("KOSM_CAND").ok().and_then(|v| v.parse().ok()).unwrap_or(400_000)
}

pub fn j_relax() -> f64 {
    std::env::var("KOSM_JRELAX").ok().and_then(|v| v.parse().ok()).unwrap_or(0.02)
}
...
pub struct Water {
    ...
    pub gpu: Option<kosm_mpm::GpuMpm>,
...
impl Water {
    ...
    pub fn fill(h: f64, dt: f64, air_above: f64, bulk: f64) -> Self {
        ...
        // KOSM_PPC particles per cell along each axis (default 2, so 8 per
        // cell): one per cell makes the density estimate too noisy and the
        // water boils
        let ppa: usize = std::env::var("KOSM_PPC").ok().and_then(|v| v.parse().ok()).unwrap_or(2);
        let sp = h / ppa as f64;
        ...
        Self {
            ...
            flip: std::env::var("KOSM_FLIP").ok().and_then(|v| v.parse().ok()).unwrap_or(0.5),
            bulk,
            time: 0.0,
            interior_mass: 0.0,
            level_offset: 0.0,
    ...
    pub fn enable_gpu(&mut self, max_subs: u32) -> Result<(), String> {
        ...
        let particles = kosm_mpm::Particles {
        ...
        self.gpu = Some(kosm_mpm::GpuMpm::new(params, &particles, max_subs)?);
        Ok(())
    }
    ...
    pub fn gpu_candidates(&self) -> Vec<Droplet> {
        ...
        if c.found > c.x.len() as u32 && std::env::var_os("KOSM_PROF").is_some() {
            println!("pick   {} candidates found, {} kept (raise KOSM_CAND)", c.found, c.x.len());
        }
        c.x.iter()
            .zip(&c.v)
    ...
    pub fn candidates(&mut self, cell: f64, above: f64, speed: f64, below: f64, up: f64) -> (HeightGrid, Vec<Droplet>) {
        if self.gpu.is_some() && std::env::var("KOSM_GPU_SURFACE").map(|v| v != "0").unwrap_or(true) {
