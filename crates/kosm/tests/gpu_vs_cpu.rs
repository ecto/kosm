...
use kosm::fluid::splash::{Body, Water};

/// The melon the pool sim drops, as a shape for the solver to push against.
const MELON_AXES: [f64; 3] = [0.15, 0.105, 0.105];
use phyz_math::Vec3;
...
fn one_and_many_substeps() {
    ...
    let air: f64 = std::env::var("KOSM_TEST_AIR").ok().and_then(|v| v.parse().ok()).unwrap_or(0.5);
    let bulk = 2.0e6;
    ...
    for &n in &[1usize, 10, 100, 1000] {
        ...
        for (a, b, x) in cpu.v.iter().zip(&gpu.v).zip(&cpu.x).map(|((a, b), x)| (a, b, x)) {
            if x.x.hypot(x.y) > kosm::fluid::box_half() - 0.15 { far_cnt += 1; far_sum += (*a - *b).norm_squared(); }
        }
...
fn settled_density_gpu() {
    let h: f64 = std::env::var("KOSM_TEST_H").ok().and_then(|v| v.parse().ok()).unwrap_or(0.025);
    let bulk = 2.0e6;
    let dt = 0.35 * h / (bulk / 1000.0f64).sqrt();
    ...
    for p in &w.x {
        let f = (p.x.abs() / kosm::fluid::POOL_X).min(0.999);
