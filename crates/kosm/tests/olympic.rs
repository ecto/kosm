...
fn seam_profile_at_impact() {
    ...
    let frame: usize = std::env::var("KOSM_TEST_FRAME").ok().and_then(|v| v.parse().ok()).unwrap_or(46);
    ...
    let mut x = 0.0;
    while x < half + 0.6 {
        let y = if std::env::var_os("KOSM_TEST_DIAG").is_some() { x } else { 0.0 };
...
fn far_field_conserves_energy() {
    ...
    if std::env::var("KOSM_FAR_NL").map(|v| v == "1").unwrap_or(false) {
