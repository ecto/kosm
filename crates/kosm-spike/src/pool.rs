...
fn melon_density() -> f64 {
    std::env::var("KOSM_MELON_RHO").ok().and_then(|v| v.parse().ok()).unwrap_or(950.0)
}
...
impl PoolSimulation {
    ...
    fn step_with_water(&mut self) {
        ...
        // KOSM_HOLD=<z>: pin the melon at that depth and just read the force
        // the water puts on it; a hydrostatic check of the coupling
        if let Some(z) = std::env::var("KOSM_HOLD").ok().and_then(|v| v.parse::<f64>().ok()) {
            self.state.q[5] = z;
            self.state.v[3] = 0.0;
            self.state.v[4] = 0.0;
            self.state.v[5] = 0.0;
...
pub fn run(out: &Path, frames: usize, width: u32, height: u32, splash: bool) -> anyhow::Result<()> {
    ...
    for k in 0..frames {
        ...
        if k == 0 || k % 5 == 0 || k + 1 == frames || std::env::var_os("KOSM_PROF").is_some() {
            ...
            if std::env::var_os("KOSM_WATER_STATS").is_some() {
                if let Some(w) = drop.water.as_mut() {
                    w.sync_from_gpu();
                }
            }
            if let (Some(w), true) = (&drop.water, std::env::var_os("KOSM_WATER_STATS").is_some()) {
