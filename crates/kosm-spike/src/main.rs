...
use kosm_spike::{audio, colliders, frame, garage, glass, lamp, light, pool, room};
use std::fs;
use std::path::Path;
...
fn export_track(doc: &Document, t: Tilt, out: &Path) -> anyhow::Result<()> {
    ...
    let spec = vcad_kernel_export::StlSpec { name: "Kosm marble track".into(), meshes };
...
fn garage_stage(level: &Level, map_dir: &Path, out: &Path) -> anyhow::Result<()> {
    ...
    if std::env::var_os("KOSM_TRACE").is_some() {
...
fn light_stage(
    ...
    // re-renders. KOSM_SWEEP=<frames> (0 to skip).
    let frames: usize = std::env::var("KOSM_SWEEP").ok().and_then(|v| v.parse().ok()).unwrap_or(0);
    if frames > 0 {
