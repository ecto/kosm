...
#[test]
fn archimedes() {
    let z: f64 = std::env::var("KOSM_TEST_Z").ok().and_then(|v| v.parse().ok()).unwrap_or(-0.35);
    let hs: Vec<f64> = std::env::var("KOSM_TEST_H")
        .ok()
        .map(|v| v.split(',').filter_map(|s| s.parse().ok()).collect())
        .unwrap_or_else(|| vec![0.05, 0.025]);
    for &h in &hs {
        hold(h, true, z);
        if std::env::var_os("KOSM_TEST_CPU").is_some() {
            hold(h, false, z);
        }
    }
}
