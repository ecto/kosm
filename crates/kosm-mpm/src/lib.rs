...
impl GpuMpm {
    ...
    fn slot_budget(params: &Params, particles: &Particles, nb: [u32; 3], nblocks: u32) -> u32 {
        ...
        let budget = std::env::var("KOSM_MAX_BLOCKS").ok().and_then(|v| v.parse().ok()).unwrap_or(n + n / 6 + 1024);
    ...
    pub fn step(&mut self, body: &Body, subs: u32) -> Vec<Reaction> {
        ...
        assert!(
            ...
            "block-sparse grid overflowed: {} active blocks of {} budgeted ({} in the box). Raise KOSM_MAX_BLOCKS.",
            a[1],
            self.max_slots,
            self.nblocks
        );
